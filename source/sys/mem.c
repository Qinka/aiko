...
void __mem_init(void) {
  memset(aiko_mem_pool.MP_pool, 0, sizeof(uint32_t) * AIKO_MEM_POOL_MAX_SIZE);
...
uint32_t __malloc(size_t s) {
  ...
  if (s == 0) // need to alloc
    return 0XFFFFFFFF;
  needed_mem_blk = s / sizeof(uint32_t);
  if (s % sizeof(uint32_t))
    needed_mem_blk++;
  for (signed long offset = AIKO_MEM_TABLE_MAX_SIZE - 1; offset >= 0;
       --offset) {
...
void free(void *p) {
  if (p == NULL)
    return;
  uint32_t offset = (uint32_t)p - (uint32_t)aiko_mem_pool.MP_pool;
  __free(offset);
}